...
11 tables across migrations `0001`–`0007`: `companies`, `users`, `jobs`, `candidates`, `interview_types`, `interviews`, `interview_feedback`, `email_logs`, `email_preferences`, `email_queue`, `refresh_tokens`.

Notable migrations: `0003` rebuilt `interviews` to make `interviewer_id` nullable (via `interviews_new` + rename), `0006` added `status` + `claimed_at` to `email_queue` for atomic dequeue, `0007` added `refresh_tokens`.
...
- Login rate limit: 5 attempts / 60s per email (KV key `rl:login:{email}`)
...
- **Rate limit** (`src/middleware/rate-limit.ts`): KV fixed window, key `rl:{CF-Connecting-IP}:{window}`, applied to all `/api/*`. Sets `X-RateLimit-*` on every response, 429 + `Retry-After` when over. Disable with `RATE_LIMIT_ENABLED = "false"`.
- **R2 budget** (`src/services/storage/r2-limits.ts`): `assertUploadAllowed()` / `assertReadAllowed()` block writes and reads before the free tier is exceeded (10 GB storage, 1M Class A, 10M Class B per month — defaults sit at 90%). Inspect live usage at `GET /api/analytics/r2-usage`.
